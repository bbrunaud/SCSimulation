...
function initialize_orders(d::SCSData, ts=d.currentperiod + 1, te=ts+d.schedulinghorizon-1; verbose=false)
    for i in d.plants
        for p in d.products
...
function update_orders(d::SCSData; verbose=false)
    ...
    initialize_orders(d, d.currentperiod+8, d.currentperiod+d.schedulinghorizon, verbose=verbose)
end
