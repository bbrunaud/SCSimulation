...
for tindex in periods
    ...
    for lindex in slots
        ...
        for index in products
            ...
            if getvalue(w[index,lindex,tindex]) == 1

                println("Product $index was produced in slot $lindex of week $tindex:")
                #println("w_sol = ", getvalue(w[index,:,:]))
                println("Θl_sol = ", getvalue(Θl[index,lindex,tindex]))
                println("ts_sol = ", getvalue(ts[lindex,tindex]))
                println("te_sol = ", getvalue(te[lindex,tindex]))
                println("tiempo total = ", getvalue(te[lindex,tindex]) - getvalue(ts[lindex,tindex]))
            end
        end
    end
